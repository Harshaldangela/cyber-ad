...
app = FastAPI(title="Improved Hybrid Cyber Awareness Ad API")
...
def improved_hybrid_spam_detection(text: str) -> Tuple[int, float]:
    """
...
async def analyze(req: AnalyzeRequest):
    try:
        # Use improved hybrid detection
        pred, confidence = improved_hybrid_spam_detection(req.text)
...
async def test_detection(req: GenerateRequest):
    ...
    try:
        ...
        hybrid_prediction, hybrid_confidence = improved_hybrid_spam_detection(req.text)
...
# Run with: uvicorn improved_hybrid_api:app --reload --port 8000