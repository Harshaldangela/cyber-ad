...
class ImprovedRuleBasedSpamDetector:
    def __init__(self):
        ...
        self.spam_keywords = [
            ...
            # Hindi spam keywords (more comprehensive)
            'बधाई', 'बधाई हो', 'जीत', 'जीते', 'जीता', 'जीतने', 'जीतें', 'जीतेगा', 'जीतेगी',
            'उपहार', 'उपहारों', 'लाख', 'लाखों', 'पैसा', 'पैसे', 'राशि', 'राशियों',
            'क्लिक', 'लिंक', 'जल्दी', 'तुरंत', 'संपर्क', 'विवरण', 'डिटेल्स', 'details',
            'जीत', 'जीता', 'जीते', 'जीतें', 'जीतेगा', 'जीतेगी', 'जीतना', 'जीतने'
        ]
        ...
        # High confidence spam patterns (more comprehensive)
        self.high_confidence_patterns = [
            ...
            r'badhai.*jite.*raashi.*click.*vivaran',
            r'बधाई.*जीत.*राशि.*क्लिक.*विवरण',
            r'बधाई.*लाख.*जीत.*क्लिक.*विवरण',
            r'जीत.*लाख.*राशि.*क्लिक.*विवरण'
        ]
    ...
    def extract_features(self, text: str) -> Dict[str, Any]:
        ...
        # Count spam keywords (more flexible matching)
        spam_keyword_count = 0
        for keyword in self.spam_keywords:
            # Use more flexible matching for Hindi text
            if keyword in text_lower:
                spam_keyword_count += 1
            # Also check for partial matches for Hindi words
        ...
        money_terms = ['free', 'cash', 'money', 'income', 'earn', 'prize', 'won', 'win', 'लाख', 'राशि', 'पैसा']
        money_count = sum(1 for word in money_terms if word in text_lower)
        
        # Check for action requests
        action_requests = ['click', 'call', 'text', 'reply', 'send', 'verify', 'apply', 'क्लिक', 'संपर्क']
    ...
    def predict(self, text: str) -> Tuple[int, float]:
        ...
        if features['spam_keyword_count'] > 0:
            spam_score += min(0.4, features['spam_keyword_count'] * 0.08)
        
        # Suspicious patterns
        if features['suspicious_pattern_count'] > 0:
            spam_score += min(0.3, features['suspicious_pattern_count'] * 0.15)
        
        # Urgency indicators
        if features['urgency_count'] > 0:
            spam_score += min(0.2, features['urgency_count'] * 0.07)
        
        # Money-related terms
        if features['money_count'] > 0:
            spam_score += min(0.25, features['money_count'] * 0.05)
        
        # Action requests
        if features['action_count'] > 0:
            spam_score += min(0.25, features['action_count'] * 0.05)
        
        # Normalize score
        spam_score = min(1.0, spam_score)
        
        # Make prediction based on threshold
        prediction = 1 if spam_score > 0.25 else 0  # Lower threshold for better sensitivity
        confidence = spam_score
        
        return prediction, confidence

# Test the improved rule-based detector
if __name__ == "__main__":
    ...
    print("Testing Improved Rule-Based Spam Detector:")
    print("=" * 50)
