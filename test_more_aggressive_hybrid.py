...
def more_aggressive_hybrid_spam_detection(text: str, _tfidf, _clf, _rule_detector):
    """
    More aggressive hybrid spam detection - trust rule-based detector more for known spam patterns
    ...
    if ml_prediction is not None:
        ...
        if ml_prediction == rule_prediction:
            ...
            # If they disagree, be more aggressive in trusting the rule-based detector
            # Especially for known spam patterns
            
            # Check if this is a known spam pattern that the rule detector recognizes
            if rule_confidence > 0.3:  # Lower threshold - even moderate confidence from rules
                # If rule-based has any meaningful confidence, trust it more
                final_prediction = rule_prediction
                # Boost confidence since we trust the rules for known patterns
                combined_confidence = min(1.0, rule_confidence + 0.2)
...
def main():
    ...
    for msg in test_messages:
        ...
        # Get more aggressive hybrid prediction
        prediction, confidence = more_aggressive_hybrid_spam_detection(msg, _tfidf, _clf, _rule_detector)
        label = "SPAM" if prediction == 1 else "NOT SPAM"
        print(f"More Aggressive Hybrid Prediction: {label}, Confidence: {confidence}")
